...
class DefaultAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        def max_value(gameState, player, depth, alpha, beta):
            ...
            if depth == DEPTH or len(actions) == 0:
                return self.evaluate(gameState, False, False, True)
            v = -10000
            actions = self.forwardPrune(gameState, actions, self.index, reverse, 2)
            for action in actions:
                if len(actions) > 1 and action == "Stop":
    ...
    def evaluate(self, gameState, isReverse, isStop, minimax=False):
        """
        ...
        if self.movesLeft < 80:
            ...
            if (teamIsRed and self.getScore(gameState) > 0) or (not teamIsRed and self.getScore(gameState) < 0):
                features = self.getFeaturesDefense(gameState, isReverse, isStop, minimax)
                weights = self.getWeightsDefensive(gameState)
            elif self.getScore(gameState) == 0:
                features = self.getFeatures(gameState, isReverse, isStop, minimax)
    ...
    def getFeatures(self, gameState, isReverse, isStop, minimax=False):
        """
    ...
    def forwardPrune(self, gameState, actions, player, reverse, num):
        ...
        for action in actions:
            ...
            evaluation = self.evaluate(successor, action == reverse, action == "Stop", True)
    ...
    def forwardPruneTheEnemy(self, gameState, actions, player, num):
        ...
        for action in actions:
            successor = gameState.generateSuccessor(player, action)
            evaluation = self.evaluate(successor, False, False, True)
            score.append((evaluation, action))
    ...
    # DEFENSE
    def getFeaturesDefense(self, gameState, isReverse, isStop, minimax):
        features = util.Counter()
        successor = gameState
        ...
        if not minimax:
            ...
            highProbDist2 = []
            for d in distributions[:4]:
                if d[0] > 0.15 and d[1] in self.ourSideAsASet:
    ...
    def getWeightsDefensive(self, gameState):
        return {'numInvaders': -1000, 'onDefense': 100, 'invaderDistance': -10, 'stop': -100, 'reverse': -2,
                'numFood': 5, 'capsuleProximity': 1, 'avoidWhenScared': -10000, 'capsuleInPlay': 14,
                'noisyClosestEnemy': -1, 'optimalDefenseTile': 2, 'distanceToFood': 1, 'distanceToLastEatenFood': 20,
                'closeToTeammate': -1, 'wrongSideOfFood': -3, 'borderPatrol': 3, 'ghostGuessDistance': 100}
...
class DefensiveReflexAgent(DefaultAgent):
    ...
    def getFeatures(self, gameState, isReverse, isStop, minimax=False):
        return self.getFeaturesDefense(gameState, isReverse, isStop, minimax)
...
class ParticleFilter:
    ...
    def __init__(self, myteam, myOpponent, gameState, numParticles=20):
    ...
    def getOppPosDistr(self, gameState, observer, enemyPos, enemyIndex):
        ...
        for a in legalMoves:
            ...
            if a is "Stop":
                ...
                isStop = False
            score = evalOpponent(observer, self.myOpponent, self.setEnemyPositions(gameState.deepCopy(), positions[a]), enemyIndex, isStop)
            # print a
            # print(score)
            if score < 0:
                actionDist[a] = 0.1
...
def evalOpponent(observer, opponent, gameState, enemyIndex, isStop):
    ...
    if gameState.isOnRedTeam(enemyIndex):
        if opponentPos[0] > gameState.getWalls().width//2:
            ...
            eval = 10000/float(min(observer.getMazeDistance(opponentPos, ourTeamPos[0]), observer.getMazeDistance(opponentPos, ourTeamPos[1]))+1)
    else:
        if opponentPos[0] < gameState.getWalls().width//2:
            foodDistance = []
            for food in observer.getFoodYouAreDefending(gameState).asList():
