...
class JointParticleFilter:
    ...
    def __init__(self, myteam, opponents, gameState, numParticles=600):
...
class ReflexCaptureAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        print(beliefs[(1,1),(1,2)])
        if gameState.isOnRedTeam:
            for pos in beliefs:
                self.debugDraw([i for i in pos],[0,0,min(beliefs[pos]*5,1)],clear=True)
        else:
            for pos in beliefs:
                self.debugDraw([i for i in pos],[min(beliefs[pos]*5,1),0,0],clear=True)
